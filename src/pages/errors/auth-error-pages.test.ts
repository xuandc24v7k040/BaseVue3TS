...
async function setup(path: string) {
  ...
  const router = createRouter({
    ...
    routes: [
      ...
      { path: '/access-denied', name: 'access-denied', component: AccessDeniedPage },
      { path: '/auth-unavailable', name: 'auth-unavailable', component: AuthUnavailablePage },
      {
...
describe('AuthUnavailable page', () => {
  it('retries and routes an authenticated BRANCH principal', async () => {
    ...
    expect(context.router.currentRoute.value.path).toBe('/branch-admin/dashboard')
  })
