...
function mountAccount(component: typeof CustomerAccountSidebar | typeof AccountOverviewPage | typeof AccountProfilePage | typeof AccountAddressesPage) {
...
describe('Member Center Phase 3', () => {
  ...
  it('renders the approved overview sections and only three stats', () => {
    ...
    expect(wrapper.findAll('section[aria-label="Thống kê tài khoản"] > *')).toHaveLength(3)
    expect(wrapper.text()).not.toContain('F-Point')
    expect(wrapper.find('[data-slot="scroll-area"]').exists()).toBe(true)
    expect(wrapper.find('.overflow-x-auto').exists()).toBe(false)
    expect(wrapper.findAll('[data-slot="scroll-area"] article')).toHaveLength(4)
  })
  ...
  it('renders profile and address source-of-truth content', () => {
    ...
    expect(addresses.findAll('input[type="checkbox"], input[type="radio"]')).toHaveLength(0)
    expect(addresses.text()).toContain('Bạn có thể thêm tối đa 10 địa chỉ giao hàng.')
  ...
  it('keeps profile and address sheet footers in two equal columns', async () => {
    ...
    expect(document.querySelector('[data-slot="sheet-footer"]')?.className).toContain('grid-cols-2')
    profile.unmount()
