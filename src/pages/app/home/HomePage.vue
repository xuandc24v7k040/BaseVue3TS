...
<template>
  <div class="w-full min-w-0 max-w-full space-y-5">
    <HomeHero />
    <div
      v-if="categoriesQuery.isPending.value"
      class="grid grid-cols-4 gap-3 rounded-xl border bg-background p-4 lg:grid-cols-8"
    >
    ...
    <div
      v-else
      class="rounded-xl border border-dashed bg-background p-6 text-center text-sm text-[var(--bookora-muted)]"
    >
      Không thể tải danh mục.
      <Button type="button" variant="link" @click="categoriesQuery.refetch()"
        >Thử lại</Button
      >
    </div>

    <RecentlyViewedSection
      subtitle="Tiếp tục xem những sản phẩm bạn đã quan tâm"
    />
    ...
    <div v-else-if="homeQuery.data.value" class="space-y-5">
      <div
        class="grid min-w-0 max-w-full grid-cols-1 gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]"
      >
        <BookSection
          title="Sách bán chạy"
          :books="homeQuery.data.value.bestSellers"
          view-all-href="/san-pham?sort=popular"
          show-controls
        />
        <MemberPromotionCard />
      </div>
      ...
      >
        <BookSection
          ...
          view-all-href="/san-pham?sort=newest"
        />
    ...
    <div
      v-else
      class="rounded-xl border border-dashed bg-background p-8 text-center"
    >
      <p class="text-sm text-[var(--bookora-muted)]">
        Không thể tải sản phẩm trang chủ.
      </p>
      <Button type="button" class="mt-3" @click="homeQuery.refetch()"
        >Thử lại</Button
      >
    </div>
  </div>
