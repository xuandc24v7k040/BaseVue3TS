...
import type { AuthMeBranchDto, AuthMeResponseDto } from '@/api/generated/models'
...
function user(type: AuthMeResponseDto['type'], branches: AuthMeBranchDto[]): AuthMeResponseDto {
  return {
...
describe('admin branch selector', () => {
  it('shows system scope and real active branches for SYSTEM users', async () => {
    const principal = user('SYSTEM', [branch(BRANCH_A, 'Cần Thơ'), branch(BRANCH_B, 'Hậu Giang')])
