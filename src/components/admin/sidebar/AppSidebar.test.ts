...
function principal(): AuthMeResponseDto {
  ...
  return {
    ...
    branchAssignments: branches.map((branch, index) => ({
      ...
      permissions:
        index === 0 ? ["products.read", "inventory.read"] : ["orders.read"],
      maxRoleLevel: 20,
    })),
    maxRoleLevel: 20,
    isSuperAdmin: false,
...
describe("permission-aware admin sidebar", () => {
  ...
  it("updates menu and selected-assignment label reactively", async () => {
    ...
    expect(wrapper.findComponent(SidebarBrand).props("brand").subtitle).toBe(
      "Nhân viên kho",
    );
    expect(
      JSON.stringify(wrapper.findComponent(SidebarNav).props("items")),
    ).toContain("inventory-list");
    expect(
      JSON.stringify(wrapper.findComponent(SidebarNav).props("items")),
    ).toContain("product-list");
    expect(
      JSON.stringify(wrapper.findComponent(SidebarNav).props("items")),
