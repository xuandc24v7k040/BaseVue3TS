...
describe("ClientHeader", () => {
  it("renders the Bookora brand, search and customer actions", async () => {
    ...
    expect(
      wrapper.find('input[placeholder="Bạn đang tìm sách gì?"]').exists(),
    ).toBe(true);
    ...
    expect(
      wrapper.get('[data-testid="branch-selector-trigger"]').text().trim(),
    ).toBe("Cần Thơ");
  });

  it("navigates to the search route with a trimmed query", async () => {
    const { router, wrapper } = await mountHeader();
    await router.push("/san-pham?page=4&author=j-k-rowling");
  ...
  it("uses a pointer cursor on every wishlist and cart click target", async () => {
    ...
    expect(wrapper.get('button[aria-label="Yêu thích"]').classes()).toContain(
      "cursor-pointer",
    );
    expect(wrapper.get('button[aria-label="Giỏ hàng"]').classes()).toContain(
      "cursor-pointer",
    );
  });
  ...
  it("does not navigate when the search query is empty", async () => {
    ...
    await wrapper
      .find<HTMLInputElement>("#desktop-book-search")
      .setValue("   ");
    await wrapper.find("form").trigger("submit");
    await flushPromises();
  ...
  it("opens the live panel, stores submitted history and closes with Escape", async () => {
    ...
    await flushPromises();
    expect(
      JSON.parse(localStorage.getItem("bookora.search-history.v1") ?? "[]"),
    ).toEqual(["Chú Thuật"]);
  ...
  it("opens the branch selector and confirms another local branch", async () => {
    ...
    await wrapper
      .find('[data-testid="branch-selector-trigger"]')
      .trigger("click");
    await flushPromises();
    ...
    const hauGiangButton = Array.from(
      document.body.querySelectorAll("button"),
    ).find((button) => button.textContent?.includes("Hậu Giang"));
  ...
  it("shows an empty state and cancel keeps the last confirmed branch", async () => {
    ...
    const cancelButton = Array.from(
      document.body.querySelectorAll("button"),
    ).find((button) => button.textContent?.trim() === "Hủy");
    cancelButton?.click();
    await flushPromises();
