...
<template>
  <section
    class="w-full min-w-0 max-w-full rounded-xl border border-[var(--bookora-border)] bg-background p-4 sm:p-5"
  >
    ...
    <div
      class="grid w-full min-w-0 max-w-full auto-cols-[minmax(210px,240px)] grid-flow-col gap-3 overflow-x-auto overscroll-x-contain px-0.5 pb-2 pt-1 [scrollbar-width:none]"
    >
      <RouterLink
        v-for="book in books"
        :key="book.id"
        :to="`/san-pham/${book.slug}`"
      ...
      >
        <time
          class="mx-auto inline-flex rounded-md bg-[var(--bookora-soft)] px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-[var(--bookora-green)]"
        >
          Phát hành {{ formatProductDate(book.releaseDate) }}
        </time>
        <div class="mt-3 flex min-h-40 items-center justify-center">
          <img
            :src="book.primaryImage.url"
            :alt="book.primaryImage.altText || `Bìa sách ${book.name}`"
            class="aspect-[2/3] h-40 w-auto max-w-full object-contain drop-shadow-md"
            width="300"
            height="450"
            loading="lazy"
          />
        ...
        <h3
          class="mt-3 line-clamp-2 min-h-10 text-sm font-semibold leading-5 transition-colors duration-200 group-hover:text-[var(--bookora-green)]"
        >
          {{ book.name }}
        </h3>
