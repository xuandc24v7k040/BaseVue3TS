/**
 * Generated by orval v8.17.0 🍺
 * Do not edit manually.
 * Bookora API
 * Contract OpenAPI chính thức cho backend Bookora. Frontend dùng cookie credentials, cookie accessToken/refreshToken và header X-CSRF-Token cho mutation cần CSRF.
 * OpenAPI spec version: 1.0
 */
import type { RoleDetailResponseDtoType } from './roleDetailResponseDtoType';
import type { RolePermissionResponseDto } from './rolePermissionResponseDto';

export interface RoleDetailResponseDto {
  /**
     * @minLength 26
     * @maxLength 26
     * @pattern ^[0-7][0-9A-HJKMNP-TV-Z]{25}$
     */
  id: string;
  code: string;
  name: string;
  /** @nullable */
  description?: string | null;
  guardName: string;
  type: RoleDetailResponseDtoType;
  level: number;
  isSystem: boolean;
  isActive: boolean;
  createdAt: string;
  updatedAt: string;
  rolePermissions: RolePermissionResponseDto[];
}
