/**
 * Generated by orval v8.17.0 🍺
 * Do not edit manually.
 * Bookora API
 * Contract OpenAPI chính thức cho backend Bookora. Frontend dùng cookie credentials, cookie accessToken/refreshToken và header X-CSRF-Token cho mutation cần CSRF.
 * OpenAPI spec version: 1.0
 */
import type { StaffAssignmentsUserResponseDtoType } from './staffAssignmentsUserResponseDtoType';

export interface StaffAssignmentsUserResponseDto {
  /**
     * @minLength 26
     * @maxLength 26
     * @pattern ^[0-7][0-9A-HJKMNP-TV-Z]{25}$
     */
  id: string;
  email: string;
  /** @nullable */
  fullName: string | null;
  /** @nullable */
  phone: string | null;
  /** @nullable */
  gender: string | null;
  /** @nullable */
  birthday: string | null;
  type: StaffAssignmentsUserResponseDtoType;
  isActive: boolean;
}
