/**
 * Generated by orval v8.17.0 🍺
 * Do not edit manually.
 * Bookora API
 * Contract OpenAPI chính thức cho backend Bookora. Frontend dùng cookie credentials, cookie accessToken/refreshToken và header X-CSRF-Token cho mutation cần CSRF.
 * OpenAPI spec version: 1.0
 */
import type { AccountDashboardLatestOrderDtoStatus } from './accountDashboardLatestOrderDtoStatus';

export interface AccountDashboardLatestOrderDto {
  /**
     * @minLength 26
     * @maxLength 26
     * @pattern ^[0-7][0-9A-HJKMNP-TV-Z]{25}$
     */
  id: string;
  orderCode: string;
  status: AccountDashboardLatestOrderDtoStatus;
  customerConfirmedReceived: boolean;
  totalAmount: number;
  placedAt: string;
  itemCount: number;
  totalQuantity: number;
  productName: string;
  /** @nullable */
  productImageUrl?: string | null;
}
