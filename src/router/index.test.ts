...
async function guard(
  ...
  branchStore = {
    isInitialized: true,
    selectedBranchId:
      store.user?.type === "BRANCH" ? "01K00000000000000000000001" : null,
    effectivePermissions:
      store.user?.type === "BRANCH" ? ["dashboard.read"] : [],
    initialize: vi.fn(),
  },
) {
...
describe("auth router policy", () => {
  ...
  it.each([["/super-admin/dashboard"], ["/branch-admin/dashboard"]])(
  ...
  it("redirects anonymous customer account access to customer login", async () => {
    await expect(
      guard("/account/profile", makeStore("anonymous")),
  ...
  it("allows a CUSTOMER into Member Center", async () => {
    await expect(
      guard("/account/profile", makeStore("authenticated", "CUSTOMER")),
    ).resolves.toBe(true);
  });
  ...
  it.each([
    ...
    await expect(
      guard("/account/profile", makeStore("authenticated", type)),
  ...
  it.each([
    ...
    "redirects authenticated %s away from customer login",
    async (type, name) => {
      await expect(
        guard("/login", makeStore("authenticated", type)),
      ).resolves.toMatchObject({ name });
    },
  );
  ...
  it.each([
    ...
    await expect(guard(path, makeStore("authenticated", type))).resolves.toBe(
      true,
    );
  });
  ...
  it.each([
    ...
    await expect(
      guard(path, makeStore("authenticated", type)),
    ).resolves.toMatchObject({
      name: "access-denied",
    });
  });
  ...
  it.each([
    ...
    "redirects authenticated %s away from login",
    async (type, name) => {
      await expect(
        guard("/admin/login", makeStore("authenticated", type)),
      ).resolves.toMatchObject({
        name,
      });
    },
  ...
  it("uses AuthUnavailable for an inconclusive protected bootstrap", async () => {
    await expect(
      guard(
        ...
        makeStore("unknown", undefined, new Error("offline")),
      ),
  ...
  it("allows a route without branch requirements in system scope", async () => {
    await expect(
      guard("/super-admin/dashboard", makeStore("authenticated", "SYSTEM")),
    ).resolves.toBe(true);
  });
  ...
  it("blocks a SYSTEM user in system scope before a branch-required page renders", async () => {
    await expect(
      guard("/branch-resource", makeStore("authenticated", "SYSTEM")),
    ).resolves.toMatchObject({
      name: "branch-required",
  ...
  it("shows branch-required state for an unassigned BRANCH user without logging out", async () => {
    ...
    await expect(
      guard("/branch-admin/dashboard", store, {
        isInitialized: true,
  ...
  it("initializes branch context after auth bootstrap and before branch validation", async () => {
    ...
    await expect(
      guard("/branch-admin/dashboard", store, branchStore),
    ).resolves.toBe(true);
  ...
  it("does not redirect-loop on the branch-required state route", async () => {
    await expect(
      guard("/branch-required", makeStore("authenticated", "SYSTEM")),
    ).resolves.toBe(true);
  });
...
describe("safe auth redirects", () => {
  ...
  it("accepts a public or customer route only for CUSTOMER principals", () => {
    expect(safeRedirectForUser(makeRouter(), "/", "CUSTOMER")).toEqual({
      path: "/",
    });
    expect(
      safeRedirectForUser(makeRouter(), "/account/profile", "CUSTOMER"),
    ).toEqual({ path: "/account/profile" });
    expect(
      safeRedirectForUser(makeRouter(), "/super-admin/dashboard", "CUSTOMER"),
    ).toBeNull();
  });
