...
const visibleGroups = computed(() =>
  filterPermissionGroups(groups.value, draftSelectedIds.value, search.value, filter.value),
)
...
function toggleGroup(resource: string): void {
  ...
  if (!group) return
  draftSelectedIds.value = togglePermissionGroup(group, draftSelectedIds.value, capabilities.value)
}
...
<template>
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent
      class="z-[70] grid max-h-[90dvh] w-[calc(100vw-1rem)] max-w-5xl grid-rows-[auto_auto_minmax(0,1fr)_auto] gap-0 overflow-hidden p-0"
    >
      <DialogHeader class="border-b px-4 py-4 sm:px-6">
        <DialogTitle class="flex items-center gap-2"
          ><ShieldCheck class="h-5 w-5" />Chọn quyền hạn</DialogTitle
        >
        <DialogDescription class="space-y-1">
          <span class="block break-words"
            >{{ roleName || 'Vai trò mới' }} ·
            <span class="font-mono">{{ roleCode || 'Chưa có mã' }}</span></span
          >
          <span class="block"
            >Đã chọn {{ draftSelectedIds.size }} quyền. Thay đổi chỉ được lưu khi submit form vai
            trò.</span
          >
        </DialogDescription>
      </DialogHeader>
      ...
      <div class="space-y-3 border-b px-4 py-3 sm:px-6">
        <div class="relative p-1">
          ...
          <Search
            class="pointer-events-none absolute left-4 top-3.5 h-4 w-4 text-muted-foreground"
          />
          <Input
      ...
      <div class="min-h-0 overflow-hidden">
        <ScrollArea class="h-full">
          ...
          <div
            v-else-if="error"
            class="m-4 rounded-lg border border-destructive/30 bg-destructive/5 p-6 text-center sm:m-6"
          >
            <p class="font-medium">Không thể tải đầy đủ dữ liệu quyền hạn.</p>
            <p class="mt-1 text-sm text-muted-foreground">
              Không thể áp dụng quyền khi catalog hoặc quyền hiện tại chưa tải xong.
            </p>
            <Button type="button" variant="outline" class="mt-4" @click="emit('retry')"
              ><RefreshCcw class="mr-2 h-4 w-4" />Thử lại</Button
            >
          </div>
          <div v-else-if="!catalog.length" class="p-8 text-center text-sm text-muted-foreground">
            Catalog chưa có quyền hạn nào.
          </div>
          <div
