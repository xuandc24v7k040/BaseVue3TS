...
const available = computed(() =>
  VARIANT_METADATA_REGISTRY.filter(
    (item) => !props.selected.includes(item.key),
  ),
);
const visible = computed(() =>
  VARIANT_METADATA_REGISTRY.filter((item) => props.selected.includes(item.key)),
);
...
<template>
  <div class="space-y-3 rounded-lg border border-dashed p-3">
    <div class="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h4 class="text-sm font-medium">Thông tin bổ sung</h4>
          <p class="text-xs text-muted-foreground">
            Xóa trường sẽ xóa giá trị khi lưu.
          </p>
        </div>
        <Popover v-model:open="open">
          <PopoverTrigger as-child
            ><Button
              ...
              variant="outline"
              :aria-expanded="open"
              ><Plus class="mr-2 h-4 w-4" />Thêm trường<ChevronDown
                class="ml-2 h-4 w-4" /></Button
          ></PopoverTrigger>
          <PopoverContent class="z-50 w-[min(92vw,22rem)] p-0" align="end">
            <Command
              ><CommandList>
                ...
                >
                  Đã thêm tất cả trường có thể dùng.
                </p>
              </CommandList></Command
            >
          </PopoverContent>
    ...
    >
        ...
        >
          <div class="flex items-center justify-between gap-2">
            <Label :for="`${idPrefix}-${item.key}`">{{ item.label }}</Label
            ><Button
              type="button"
              size="icon-sm"
              variant="ghost"
              :aria-label="`Xóa trường ${item.label}`"
          ...
          <Input
            ...
            @update:model-value="emit('update:value', item.key, String($event))"
          />
        </div>
    </div>
  </div>
