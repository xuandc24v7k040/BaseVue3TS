...
<template>
  <Card class="min-w-0 overflow-hidden">
    ...
    <CardContent class="min-h-0 min-w-0 space-y-5 overflow-hidden">
      ...
      <template v-if="!readonly">
        ...
        <ScrollArea v-if="queue.items.length" class="h-[min(18rem,45vh)] min-h-0 min-w-0 rounded-lg border"><div class="grid min-w-0 gap-2 p-3 lg:grid-cols-2"><article v-for="item in queue.items" :key="item.id" class="flex min-w-0 overflow-hidden gap-3 rounded-md border p-2"><img v-if="item.previewUrl" :src="item.previewUrl" alt="Ảnh đang chờ tải" class="h-16 w-16 shrink-0 rounded object-cover"><div class="min-w-0 flex-1 overflow-hidden"><p class="truncate text-sm font-medium" :title="item.file.name">{{ item.file.name }}</p><p class="text-xs text-muted-foreground">{{ item.state }}</p><p v-if="item.error" class="break-words text-xs text-destructive">{{ item.error }}</p><div class="mt-1 flex flex-wrap gap-1"><Button v-if="item.state === 'ERROR_RETRYABLE'" type="button" size="sm" variant="outline" @click="queue.uploadItem(item).then(refresh)">Thử lại</Button><Button v-if="item.state !== 'UPLOADING'" type="button" size="sm" variant="ghost" @click="queue.remove(item.id)">Bỏ</Button></div></div></article></div></ScrollArea>
      </template>
    </CardContent>
  </Card>
