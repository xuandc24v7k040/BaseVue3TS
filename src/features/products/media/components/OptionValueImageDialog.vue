...
<template>
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="flex max-h-[90dvh] flex-col gap-0 p-0 sm:max-w-xl">
      ...
      <ScrollArea class="min-h-0 flex-1"><div class="space-y-3 p-5"><ImageDropzone v-model="file" :current-url="removeCurrent ? null : value.imageUrl" :disabled="pending" :image-alt="`Thumbnail ${value.label}`" @remove="removeCurrent = true" @invalid="error = $event" @valid="error = ''; removeCurrent = false" /><p v-if="error" class="text-sm text-destructive">{{ error }}</p><p v-if="option.presentationType === 'IMAGE' && !value.imageUrl && !file" class="text-sm text-muted-foreground">Chưa có ảnh đại diện cho giá trị này.</p></div></ScrollArea>
