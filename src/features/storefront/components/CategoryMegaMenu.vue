...
const activeCategory = computed(
  () =>
    props.categories.find((category) => category.slug === activeSlug.value) ??
    props.categories[0] ??
    null,
);
...
function toggle(): void {
  ...
  if (open.value && !activeSlug.value)
    activeSlug.value = props.categories[0]?.slug ?? null;
}
...
<template>
  ...
  >
    ...
    >
      <div
        class="border-r border-[var(--bookora-border)] bg-[var(--bookora-cream)] p-3"
      >
        <RouterLink
          ...
          :class="
            activeCategory?.id === category.id
              ? 'bg-[var(--bookora-soft)] text-[var(--bookora-green)]'
              : ''
          "
      ...
      <div class="p-5">
        <div
          v-if="activeCategory"
          class="flex items-center justify-between border-b border-[var(--bookora-border)] pb-3"
        >
          <h2 class="text-lg font-bold">{{ activeCategory.name }}</h2>
          <RouterLink
            :to="`/san-pham?category=${activeCategory.slug}`"
            class="text-sm font-semibold text-[var(--bookora-green)] hover:underline"
            @click="close()"
          >
            Xem tất cả →
          </RouterLink>
        ...
        <div
          ...
          class="grid grid-cols-2 gap-1 pt-3"
        >
          <RouterLink
        ...
        <p v-else class="py-8 text-center text-sm text-[var(--bookora-muted)]">
