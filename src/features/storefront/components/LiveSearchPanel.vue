...
function clearHoveredIndex(index?: number): void {
  if (index === undefined || hoveredIndex.value === index) hoveredIndex.value = -1;
}
</script>
  ...
  >
    ...
    >
      <ScrollArea type="auto" class="h-full">
        <div
          class="grid min-h-0"
          :class="mobile ? 'grid-cols-1' : 'md:grid-cols-[minmax(0,1.65fr)_minmax(230px,0.85fr)]'"
        >
      <div class="min-w-0 p-4 sm:p-5">
        ...
        >
          ...
          <Button type="button" variant="link" class="mt-1" @click="emit('retry')">
            Thử lại
          </Button>
        </div>
        ...
        >
          <div>
            ...
            <p class="mt-2 text-sm font-medium">Nhập ít nhất 2 ký tự để tìm sách.</p>
          </div>
        </div>
          ...
          >
            ...
            <span class="min-w-0 self-center">
              <span class="flex min-w-0 items-start gap-2">
                ...
                >
                  <template
                    v-for="(segment, segmentIndex) in highlightSearchText(product.name, query)"
              ...
              <strong class="mt-1 block text-sm text-[var(--bookora-green)]">
      ...
      >
        <div class="flex items-center justify-between gap-3">
          <h2 class="flex items-center gap-2 text-sm font-bold">
            <Clock3 class="size-4 text-[var(--bookora-muted)]" /> Tìm kiếm gần đây
          </h2>
          <button
            v-if="history.length"
            type="button"
        ...
        >
          <Sparkles class="size-4 text-[var(--bookora-green)]" /> Gợi ý tìm kiếm
        </h2>
        <div v-if="visibleSearchSuggestions.length" class="mt-3 flex flex-wrap gap-2">
          <button
    ...
    >
      Xem tất cả {{ total > suggestions.length ? `${total} ` : "" }}kết quả cho “{{ query }}” →
    </button>
  </section>
</template>
