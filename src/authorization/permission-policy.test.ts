...
function principal(
  options: Partial<AuthMeResponseDto> = {},
): AuthMeResponseDto {
  return {
...
describe("permission policy", () => {
  ...
  it("supports exact can/canAny/canAll semantics", () => {
    ...
    expect(policy.can("staff" as typeof ADMIN_PERMISSIONS.STAFF_READ)).toBe(
      false,
    );
  ...
  it("fails closed before initialization and never bypasses from SYSTEM type alone", () => {
    ...
    expect(
      createPermissionPolicy(system, null).can(ADMIN_PERMISSIONS.USERS_READ),
    ).toBe(false);
    expect(
      createPermissionPolicy(system, {
        isInitialized: false,
        effectivePermissions: [],
      }).can(ADMIN_PERMISSIONS.USERS_READ),
    ).toBe(false);
    expect(
      createPermissionPolicy(system, {
        isInitialized: true,
        effectivePermissions: [],
      }).can(ADMIN_PERMISSIONS.STAFF_READ),
    ).toBe(false);
  });
