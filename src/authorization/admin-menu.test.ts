...
import { resolveFirstAllowedAdminRoute, resolveVisibleAdminMenu } from '@/authorization/admin-menu'
...
function policy(type: AuthMeResponseDto['type'], permissions: string[], isSuperAdmin = false) {
  const principal = {
    type,
...
function visibleIds(type: AuthMeResponseDto['type'], permissions: string[]): string[] {
  return resolveVisibleAdminMenu(type, policy(type, permissions))
...
describe('admin menu and safe landing', () => {
  ...
  it('lands CASHIER on orders and gives empty STAFF no admin module', () => {
    expect(resolveFirstAllowedAdminRoute(
      'BRANCH',
      policy('BRANCH', [ADMIN_PERMISSIONS.ORDERS_READ, ADMIN_PERMISSIONS.PAYMENTS_CREATE]),
      true,
    )).toEqual({ name: 'branch-admin-orders' })
    expect(resolveFirstAllowedAdminRoute('BRANCH', policy('BRANCH', []), true)).toBeNull()
  })
  ...
  it('hides empty groups and has no fake category, payment, or report item', () => {
    const items = resolveVisibleAdminMenu('BRANCH', policy('BRANCH', [ADMIN_PERMISSIONS.ORDERS_READ]))
